					...
					<form id="despegue-form">
						...
						<div class="pull-right" style="margin-right: 20px;">
							...
							<div class="form-group" style="margin-top: 20px;">
								<label><strong>Clientes FBO: </strong></label>
								<div class="input-group">
									<?php
									// Prepend the "No Asignar FBO" option
									$options = ['' => '-- No Asignar FBO --'] + $clientefbo;
									?>
									{!! Form::select('clientefbo', $options, null, [ // Use $options here
									'class' => 'form-control',
									'id' => 'clientefbo-select'
									//
									]) !!}
								</div>
							</div>
...
<script>
	...
	$(document).ready(function() {
		...
		$('#otros_cargos-select').chosen({
			width: '450px'
		});
		$('#piloto_id-select').chosen({
			width: '200px'
		});
		$('#puerto_id-select').chosen({
			width: '140px'
		});
		$('#cliente_id-select').chosen({
			width: '500px'
		});
		...
		*/
		...
		//Datemask dd/mm/yyyy
		$('#fecha-datepicker').inputmask("dd/mm/yyyy", {
