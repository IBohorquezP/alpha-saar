...
class FacturaController extends Controller
{
    ...
    protected function crearFactura($factura, $output = 'I', $dir = 'facturas/')
    {
        ...
        $clienteFbo = null;
        if ($despegue && $despegue->clientefbo) {
            $clienteFbo = \App\Cliente::find($despegue->clientefbo);
        }
        ...
        if ($despegue) {
            $pdf->SetFont('helvetica', '', '10', true);
            $html = view('pdf.dosa', compact('factura', 'despegue', 'traductor', 'clienteFbo'))->render();
        } else {
            $pdf->SetFont('helvetica', '', '10', '', true);
...
*/
    ...
    protected function getFacturaDetallesDataFromRequest($request)
    {
        ...
        for ($i = 0; $i < $size; $i++) {
            $facturaDetallesData[] = array(
                'concepto_id'       => $detalles["concepto_id"][$i],
